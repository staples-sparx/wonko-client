(ns wonko-client.collectors.postgresql.queries)
...
(def all-queries
  ...
  {:cache-tables
   ...
   ;; Disk usage
   :table-sizes
   {:query "SELECT relname AS relation, pg_total_relation_size(C.oid) AS total_size
            FROM pg_class C
   ...
   :relation-sizes
   {:query "SELECT relname AS relation, pg_relation_size(C.oid) AS size
            FROM pg_class C
            LEFT JOIN pg_namespace N ON (N.oid = C.relnamespace)
   ...
   ;; Bloat
   :table-bloat
   {:query "SELECT tblname as relation, bs*tblpages AS real_size,
   ...
   {:query "SELECT
            ...
            ORDER BY 1,2;"
    :properties #{:relation :indexname}}

   ;; Tuples
   :tuple-info
   {:query "SELECT relname as relation, EXTRACT (EPOCH FROM current_timestamp-last_autovacuum) as since_last_av,
   ...
   ;; queries
   :queries
   {:query "SELECT query, calls, total_time, rows,
            shared_blks_hit,local_blks_hit, blk_read_time, blk_write_time
   ...
   ;; connections
   :conn-states
   {:query "SELECT state, count(*) AS connection_count
            FROM pg_stat_activity GROUP BY state;"
    :properties #{:state}}

   :conn-waits
   {:query "SELECT waiting, count(*) AS connection_count
